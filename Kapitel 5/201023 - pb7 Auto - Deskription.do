...
***********************************************************
***********************************************************
* Spezielle Datenaufbereitung für Beispiel "Entscheidung Auto (= pb7)"

* Variable "pb7_fail" generieren, die den Eintritt des Ereignises widergibt

gen pb7_fail = 0

move pb7_fail eb_beginn1

replace pb7_fail = 1 if pb_dauer==pb7_risktime & pb7_event==1

drop if pb7_event==1 & pb_dauer>pb7_risktime

tab pb7_fail if pb7_fail==1
...
stset pb_end, id(id) time0(pb_start) origin(time pb_start) failure(pb7_fail==1)
...
* Mediandauer bis Realisierung Erwerb eines Autos

sum pb_dauer if pb7_event==1, detail

***********************************************************

* Deskriptive Statistiken

stsum if pb7_event==1
stdes if pb7_event==1
...
sts graph if pb_dauer <= 180 & pb7_event==1,							/// restricting X-range to 0 till 120
fail 																	/// graph starts at the bottom of the >-axis instead of the top
title("(e) Erwerb eines Autos (471)") 									/// title
...
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb7 - auto - km_plot.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb7 - auto - km_plot.tif", replace width(2000)	// format: TIFF (for upload) 
...
sts graph if pb_dauer <= 180,				 								/// restricting X-range to 0 till 120
...
legend(label(1 Unbefristet (405)) label(2 Befristet (66); Log-rank: 1,47))	// Label legend

* Export graph

graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb7 - auto - km_plot + logrank.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb7 - auto - km_plot + logrank.tif", replace width(2000)	// format: TIFF (for upload)