...
***********************************************************
***********************************************************
* Spezielle Datenaufbereitung für Beispiel "Entscheidung Haushaltskasse (= pb5)"

* Variable "pb5_fail" generieren, die den Eintritt des Ereignises widergibt

gen pb5_fail = 0

move pb5_fail eb_beginn1

replace pb5_fail = 1 if pb_dauer==pb5_risktime & pb5_event==1

drop if pb5_event==1 & pb_dauer>pb5_risktime

tab pb5_fail if pb5_fail==1
...
stset pb_end, id(id) time0(pb_start) origin(time pb_start) failure(pb5_fail==1)
...
* Mediandauer bis Realisierung Einrichtung einer Haushaltskasse

sum pb_dauer if pb5_event==1, detail

***********************************************************

* Deskriptive Statistiken

stsum if pb5_event==1
stdes if pb5_event==1
...
sts graph if pb_dauer <= 180 & pb5_event==1, /// restricting X-range to 0 till 180
fail /// graph starts at the bottom of the >-axis instead of the top
title("(c) Einrichtung einer Haushaltskasse (578)") /// title
...
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb5 - kasse - km_plot.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb5 - kasse - km_plot.tif", replace width(2000)	// format: TIFF (for upload)
...
legend(label(1 Unbefristet (488)) label(2 Befristet (90); Log-rank: 0,89)) // Label legend

* Export graph

graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb5 - kasse - km_plot + logrank.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb5 - kasse - km_plot + logrank.tif", replace width(2000)	// format: TIFF (for upload)