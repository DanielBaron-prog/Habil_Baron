...
* Homosexuelle Paare ausschließen
...
***********************************************************
***********************************************************
* Spezielle Datenaufbereitung für Beispiel "Entscheidung Heirat (= pb1)"

* Variable "pb1_fail" generieren, die den Eintritt des Ereignises widergibt

gen pb1_fail = 0

move pb1_fail eb_beginn1

replace pb1_fail = 1 if pb_dauer==pb1_risktime & pb1_event==1

drop if pb1_event==1 & pb_dauer>pb1_risktime

tab pb1_fail if pb1_fail==1
...
stset pb_end, id(id) time0(pb_start) origin(time pb_start) failure(pb1_fail==1)
...
* Mediandauer bis Realisierung Heirat

sum pb_dauer if pb1_event==1, detail

***********************************************************

* Deskriptive Statistiken

stsum if pb1_event==1
stdes if pb1_event==1
...
sts graph if pb_dauer <= 180 & pb1_event==1, 								/// restricting X-range to 0 till 120
fail 																		/// graph starts at the bottom of the >-axis instead of the top
title("(f) Heirat (543)") /// title
xtitle("Partnerschaftsdauer (Monate)") /// X-title
ytitle("Kumulierte Anteile realisierter Ereignisse (n=597)") 				/// Y-title
xlabel(0 12 24 36 48 60 72 84 96 108 120 132 144 156 168 180) 				/// X-labels
ylabel(0 "0%" .25 "25%" .5 "50%" .75 "75%" 1 "100%", angle(0)) 				// Y-labels

* Export graph

graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb1 - heirat - km_plot.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb1 - heirat - km_plot.tif", replace width(2000)	// format: TIFF (for upload)

***********************************************************

* KM-Schaetzer fuer Vergleich Befristete vs. Unbefristete
...
legend(label(1 Unbefristet (474)) label(2 Befristet (69); Log-rank: 7,52**)) // Label legend

* Export graph

graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb1 - heirat - km_plot + logrank.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb1 - heirat - km_plot + logrank.tif", replace width(2000)	// format: TIFF (for upload)