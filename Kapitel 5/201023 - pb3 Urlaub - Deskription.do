...
***********************************************************
***********************************************************
* Spezielle Datenaufbereitung für Beispiel "Entscheidung Urlaub (= pb3)"

* Variable "pb3_fail" generieren, die den Eintritt des Ereignises widergibt

gen pb3_fail = 0

move pb3_fail eb_beginn1

replace pb3_fail = 1 if pb_dauer==pb3_risktime & pb3_event==1

drop if pb3_event==1 & pb_dauer>pb3_risktime

tab pb3_fail if pb3_fail==1
...
stset pb_end, id(id) time0(pb_start) origin(time pb_start) failure(pb3_fail==1)
...
* Mediandauer bis Realisierung Erster gemeinsamer Urlaub

sum pb_dauer if pb3_event==1, detail

***********************************************************

* Deskriptive Statistiken

stsum if pb3_event==1
stdes if pb3_event==1
...
sts graph if pb_dauer <= 180 & pb3_event==1, /// restricting X-range to 0 till 180
fail /// graph starts at the bottom of the >-axis instead of the top
title("(a) Erster gemeinsamer Urlaub (895)") /// title
...
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb3 - urlaub - km_plot.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb3 - urlaub - km_plot.tif", replace width(2000)	// format: TIFF (for upload)
...
legend(label(1 Unbefristet (846)) label(2 Befristet (149); Log-rank: 0,02)) // Label legend

* Export graph

graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb3 - urlaub - km_plot + logrank.png", replace width(2000)	// format: PNG (for manuscript)
graph export "E:\Habilschrift\Analysen Kap5 - Methoden und Deskription\Stata Plots\pb3 - urlaub - km_plot + logrank.tif", replace width(2000)	// format: TIFF (for upload)